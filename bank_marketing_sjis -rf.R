...
tel_count
#成約した人
subcribed <- conf_table[4]
subcribed
#ROI
roi <- (subcribed * 5000) - (tel_count * 500)
roi
#2625500
...
#Prediction
#score<-predict(mymodel, validation_data, type = "prob")[,2]
pred_rf<- predict(mymodel, validation_data, type = "prob")
#pred_rf<- predict(rf_model,test, type="prob")
pred_rf <- pred_rf[,2]
pred <- prediction(pred_rf, validation_data$y)

perf_res <- ROCR::performance(pred, measure = "tpr", x.measure = "fpr")
plot(perf_res)
